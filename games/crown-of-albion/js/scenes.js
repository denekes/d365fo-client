...
const MapScene = {
  ...
  update(dt) {
    ...
    // dust motes drifting up through the warm light
    if (Math.random() < 0.12) {
      spawn(rnd(60, 460), rnd(MAPY + 420, MAPY + 920), {
        vx: rnd(-6, 7), vy: rnd(-16, -5), g: 0, life: rnd(2.2, 4.2),
        size: rnd(1.4, 3), color: 'rgba(255,240,200,0.55)', shrink: true,
      });
    }
    if (this.aiQueue) {
      this.aiTimer -= dt;
  ...
  openJoust() {
    ...
    Modal.show({
      ...
      buttons: [
        ...foes.map(f => ({ label: f.name, fn: () => this.pickWager(f) })),
        { label: 'Never Mind', fn: () => {} },
      ],
    });
  },
  ...
  // soft clouds drifting above the realm, offset from their shadows for parallax
  drawClouds() {
    for (let i = 0; i < 3; i++) {
      const x = ((gTime * (8 + i * 3) + i * 320) % (W + 560)) - 280 - 22;
      const y = MAPY + 180 + i * 290 - 30;
      ctx.save();
      ctx.globalAlpha = 0.10;
      ctx.fillStyle = '#f4f1ea';
      ctx.beginPath();
      ctx.ellipse(x, y, 190, 68, 0.2, 0, TAU);
      ctx.ellipse(x + 120, y + 32, 120, 50, -0.2, 0, TAU);
      ctx.ellipse(x - 92, y + 20, 90, 40, 0.1, 0, TAU);
      ctx.fill();
...
const BattleScene = {
  ...
  update(dt) {
    // drifting battlefield dust/smoke for atmosphere
    if (Math.random() < 0.5) {
      spawn(rnd(40, W - 40), rnd(620, 1040), {
        vx: rnd(-10, 10), vy: rnd(-22, -8), g: -4, life: rnd(1.6, 3.2),
        size: rnd(3, 9), color: `rgba(150,135,110,${rnd(0.05, 0.14).toFixed(3)})`, shrink: false,
      });
    }
  ...
  drawHost(cx, dir, color, soldiers, knights) {
    ...
    if (total <= 0) return;
    // war banners planted behind the host
    for (let b = 0; b < 2; b++) {
      const bx = cx + dir * (b * 130 + 30), by = 702;
      ctx.strokeStyle = '#3a2c1c'; ctx.lineWidth = 4;
      ctx.beginPath(); ctx.moveTo(bx, by); ctx.lineTo(bx, by - 100); ctx.stroke();
      const wv = Math.sin(gTime * 3 + b * 2) * 5;
      ctx.fillStyle = color;
      ctx.beginPath();
      ctx.moveTo(bx, by - 100);
      ctx.quadraticCurveTo(bx + dir * 26, by - 104 + wv, bx + dir * 48, by - 96 + wv);
      ctx.lineTo(bx + dir * 48, by - 70 + wv * 0.5);
      ctx.quadraticCurveTo(bx + dir * 26, by - 76 + wv, bx, by - 70);
      ctx.closePath(); ctx.fill();
      ctx.fillStyle = shade(color, 0.32); ctx.fillRect(bx + dir * 18, by - 94, 4, 18);
    }
    ...
    for (let row = 0; row < 3 && n < icons; row++) {
      for (let col = 0; col < 4 && n < icons; col++, n++) {
        const ph = n * 1.3;
        const bob = Math.sin(gTime * 2.2 + ph) * 2.2;
        const sway = Math.cos(gTime * 1.4 + ph) * 1.4 * dir;
        const x = cx + dir * (col * 60) + (row % 2) * 24 * dir + sway;
        const y = 712 + row * 96 + bob;
        const sc = 1 + row * 0.07;          // nearer ranks loom slightly larger
        if (n < kIcons) drawMiniKnight(x, y, 2.0 * sc, color, dir, this.lunge);
        else drawSoldier(x, y, 2.2 * sc, color, dir, this.lunge);
      }
    }
  },
  onTap() {
...
const RaidScene = {
  ...
  nextPrompt() {
    ...
    this.window = Math.max(0.7, 1.4 - this.round * 0.08);
    this.t = 0;
    this._sw = null;
  },
  resolve(ok) {
  ...
  finish() {
    ...
    let lines;
    if (won) {
      // never rob more gold than the foe actually has
      const loot = Math.min(this.foe.gold, Math.max(15, Math.round(this.foe.gold * 0.45)));
      this.foe.gold -= loot;
      p.gold += loot;
      p.fame += 6;
      lines = loot > 0
        ? `The guard yields! You escape over the wall with ${loot} gold from the coffers of ${this.foe.name}.`
        : `The guard yields — but the coffers of ${this.foe.name} are all but empty. At least the tale will be told.`;
      Sfx.fanfare();
    } else {
  ...
  /* the raid classifies gestures itself: the global tap/swipe thresholds
     leave dead zones (24–50px moves, slow swipes) that read as misses */
  onDown(x, y) { this._sw = { x, y }; },
  onUp(x, y) {
    if (this.phase !== 'prompt' || !this._sw) return;
    const dx = x - this._sw.x, dy = y - this._sw.y;
    this._sw = null;
    let g;
    if (Math.hypot(dx, dy) < 26) g = 'tap';
    else if (Math.abs(dx) >= Math.abs(dy)) g = dx > 0 ? 'right' : 'left';
    else g = 'vertical';
    this.resolve(g === this.prompt);
  },
};
